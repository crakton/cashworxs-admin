...
export const addUser = createAsyncThunk('userManagement/addUser', async (userData: NewUser, { rejectWithValue }) => {
	try {
		const token = Cookies.get('auth_token');
	...
	async (userId: string, { rejectWithValue, getState }: any) => {
		try {
			...
			const response = await axios.patch(
				`${API_URL}/roles/users/${userId}/status`,
				{
...
export const deleteUser = createAsyncThunk('userManagement/deleteUser', async (userId: string, { rejectWithValue }) => {
	try {
		const token = Cookies.get('auth_token');
...
export const fetchStates = createAsyncThunk('userManagement/fetchStates', async (_, { rejectWithValue }) => {
	try {
		const token = Cookies.get('auth_token');
